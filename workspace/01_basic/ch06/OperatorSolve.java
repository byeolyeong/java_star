...
//        c = a / (double) b;
//        System.out.println("10 / 3 결과 (실수): " + c); // 3.3333333333333335

        double divResult = (double) a / b; // 이 부분을 수정하세요.      double 을 이용한 자동 형변환
        System.out.println("10 / 3 결과 (실수): " + divResult); // 3.3333333333333335

        // [문제 1-2] 변수 a를 b로 나눈 '나머지'를 구하여 출력하세요.
//        d = a % b;
//        System.out.println("10을 3으로 나눈 나머지: " + d);

        int modResult = a % b; // 이 부분을 수정하세요.      % 산술 연산자를 이용한 나머지 계산
        System.out.println("10을 3으로 나눈 나머지: " + modResult);

        System.out.println("\n=== [실습 2] 삼항 연산자와 비교 연산자 ===");
        int number = 15;

        // ber가 [문제 2] 변수 num'짝수'인지 '홀수'인지 판별하여 결과를 문자열로 반환하고 출력하세요.
        // 힌트: 2로 나눈 나머지가 0이면 '짝수', 아니면 '홀수'입니다. 삼항 연산자(조건식 ? 참 : 거짓)를 사용하세요.
        String evenOrOdd = (number % 2 == 0) ? "짝수" : "홀수"; // 이 부분을 수정하세요.     ? : 삼항 연산자를 이용한 참, 거짓 구별
        ...
        boolean isLeapYear = ((year % 4 == 0) && (year % 100 != 0)) || (year % 400 == 0); // 이 부분을 수정하세요.    논리 연산자(&&, ||)를 이용한 참, 거짓 구별
