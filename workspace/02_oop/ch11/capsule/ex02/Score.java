...
// 기본 생성자 대신 각 점수를 전달받는 생성자를 수정
// 필드는 private로 지정해서 외부의 직접 접근을 제한함
public class Score {
    private int kor;
    private int eng;
    private int math;
    ...
    public void setMath(int math){  // 수학 점수를 새로운 값으로 교체한다
        if(math > 100 || math < 0){
            System.out.println("kor 값은 0 ~ 100 사이로 입력해야합니다.");
        } else {
            this.math = math;   //
        }
    }
    ...
    Score(int kor, int eng, int math){ // 생성자
        if(kor > 100 || kor < 0){
            System.out.println("kor 값은 0 ~ 100 사이로 입력해야합니다.");
        } else {
            this.kor = kor;
...
//        int sum = kor + eng + math;
        ...
        return result; // 미리 만들어놓은 sum을 이용함
    }
